...
func TestRouter_CircuitBackoffIncreasesAndCaps(t *testing.T) {
	r := &Router{
		policy: fallbackPolicy{
			circuitFailures: 1,
			circuitOpen:     30 * time.Second,
			circuitMaxOpen:  2 * time.Minute,
		},
	...
	before := time.Now()
	r.recordFailure("deepseek-v4-pro")
	first := r.breakers["deepseek-v4-pro"].openUntil.Sub(before)
	...
	before = time.Now()
	r.recordFailure("deepseek-v4-pro")
	second := r.breakers["deepseek-v4-pro"].openUntil.Sub(before)
	...
	before = time.Now()
	r.recordFailure("deepseek-v4-pro")
	third := r.breakers["deepseek-v4-pro"].openUntil.Sub(before)
	if third < 2*time.Minute || third > 2*time.Minute+time.Second {
		t.Fatalf("third open duration = %v, want capped at about 2m", third)
	}
}

func TestRouter_CircuitJitterStaysInRange(t *testing.T) {
	r := &Router{
		policy: fallbackPolicy{
			circuitFailures: 1,
			circuitOpen:     100 * time.Second,
			circuitMaxOpen:  5 * time.Minute,
			circuitJitter:   0.2,
		},
		breakers: make(map[string]*breakerState),
	...
	before := time.Now()
	r.recordFailure("deepseek-v4-pro")
	got := r.breakers["deepseek-v4-pro"].openUntil.Sub(before)
	if got < 80*time.Second || got > 120*time.Second {
		t.Fatalf("open duration = %v, want within 100s +/-20%%", got)
	}
}

func TestRouter_CircuitOpenExpiryKeepsOpenCountUntilSuccess(t *testing.T) {
	r := &Router{
		policy: fallbackPolicy{
			circuitFailures: 1,
			circuitOpen:     30 * time.Second,
	...
	if r.isCircuitOpen("deepseek-v4-pro") {
		t.Fatal("isCircuitOpen = true after expiry, want false")
	}
	b := r.breakers["deepseek-v4-pro"]
	if b.failures != 0 {
	...
	r.recordSuccess("deepseek-v4-pro")
