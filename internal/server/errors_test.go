...
// JSON `message` field must NOT carry the original cause, because that
// cause is built from `cause.Error()` strings (upstream/http.go's
// LowLevelError) which can embed upstream IPs, in-cluster hostnames,
// vendor body fragments, or secrets in vendor error envelopes.
//
// Operator-side detail is unchanged — rec.Kind on the audit row + the
// slog stream where the failure is observed still carry the full cause.
...
func TestErrorPayload_TransportKindsCollapseToGeneric(t *testing.T) {
	cases := []struct {
		...
		{
			name: "upstream kind erases vendor body fragment / cluster DNS name",
			err: &llmtypes.Error{
				Kind:    llmtypes.KindUpstream,
				Message: "upstream 500: backend host vendor-internal-12.svc.cluster.local refused",
			},
			wantStatus:  http.StatusBadGateway,
			wantMessage: "upstream unavailable",
		},
		{
			name: "empty kind erases vendor pod IP",
			err: &llmtypes.Error{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Defense-in-depth: even if wantMessage matches, the wire
			// must not contain any fragment that came from the original
			// cause. A regression that re-introduces concatenation would
			// trip these checks before reaching production.
			leakNeedles := []string{
				"13.226.42.85",
				"opencode.ai",
				"cluster.local",
			...
			// The error `type` field still carries the kind so callers
			// can branch on the failure category without seeing detail.
...
// kinds where the message IS the contract (caller-actionable info)
// must keep flowing to the wire so the caller can fix the request.
// bad_request needs the parse error; rate_limit needs the limit hit;
// content_filter needs the block reason.
