...
- **정책은 env** — 폴백 적격 에러 (`LLMGATE_FALLBACK_ON`), 회로 차단 (`LLMGATE_CIRCUIT_FAILURES` / `_OPEN_DURATION` / `_MAX_OPEN_DURATION` / `_JITTER`), 타임아웃 (`LLMGATE_REQUEST_TIMEOUT` / `_COMPLETE_TIMEOUT` / `_STREAM_START_TIMEOUT` / `_STREAM_IDLE_TIMEOUT`) 모두 env. 거의 바꾸지 않으므로 코드에 합리적 디폴트를 박고 env override 만 둔다. yaml 에는 정책이 살지 않는다. 전체 목록은 `docs/architecture.md` 의 정책 env 표.
