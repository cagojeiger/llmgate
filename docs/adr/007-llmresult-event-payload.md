# ADR 007: llm.result.finalized 본문 정책은 consumer 단위 capture 모드로 정의한다
...
`llm.result.finalized` event는 현재 request의 `messages`와 response의 `choices`를 그대로
NATS에 발행한다. 프롬프트와 응답이 운영자 손에 닿는 NATS 보관소까지 흘러가므로
컨슈머가 PII나 키를 본문에 담는 순간 그대로 누출된다. 반대로 컨슈머 입장에서는
재현·재처리·품질 분석을 위해 본문이 필수일 때가 있어서 "그냥 끈다"는 답도 안전망이
아니다.

스트리밍 경로(`assembly/stream_response.go`)는 모든 chunk를 메모리에 누적해 finalized
event를 만든다. 본문 정책이 없으면 누적 크기와 truncate 규칙도 정할 근거가 없고,
NATS `max_payload`(1 MB) 초과 시 손실이 일어난다.

세 가지 시나리오가 운영 안에 공존한다.
1. 본문 없이 메타데이터·usage만 필요한 컨슈머 (감사·과금).
2. 본문이 평문으로 필요한 컨슈머 (현재 동작, 분석·재처리).
3. 본문이 필요하지만 NATS 보관자에게는 가려야 하는 컨슈머 (정책·규제).
...
- **암호화 방식 / 키 관리는 미정.** age (X25519+ChaCha20-Poly1305) 또는 Vault Transit이
  후보이며, `encrypted` 모드 구현 시점에 별도 ADR로 결정한다.
- **MsgId 정책은 이 ADR의 범위 밖.** 클라이언트 제어 `X-Request-Id`가 NATS dedup 키로
  쓰이는 문제는 별도 변경에서 server-generated ULID로 분리한다.
...
- `Event` schema에 `body_capture` 필드를 새로 둔다. 다운스트림은 이 값을 보고 본문 존재
  여부와 형태를 알 수 있다.
- 누적 상한·truncate 마커·MsgId 분리는 후속 PR로 이어진다. 이 ADR은 그 PR들이 어떤
