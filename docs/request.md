...
Handler 가 200 OK 를 커밋한 뒤에는 http/stream relay 가 SSE 전송. 이벤트 사이 idle 은
...
event 를 emit 한다. 기본 wiring 은 `platform/telemetry/slog` sink 가 audit / call 로그 라인을 stdout 으로 라우팅한다.
