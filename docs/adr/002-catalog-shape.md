...
- **모델 = yaml 1 개 = endpoint 1 개** — `catalog/models/<id>.yaml` 한 파일에 id / vendor / type / base_url / auth_env 가 들어간다. 같은 vendor model name 을 다른 auth_env 로 두 번 등록해도 catalog 안에서 별개 endpoint 가 된다.
...
- **정책은 env** — `LLMGATE_FALLBACK_ON`, `LLMGATE_CIRCUIT_FAILURES`, `LLMGATE_CIRCUIT_OPEN_DURATION`. 거의 바꾸지 않으므로 코드에 합리적 디폴트를 박고 env override 만 둔다. yaml 에는 정책이 살지 않는다.
