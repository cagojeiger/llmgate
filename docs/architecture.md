...
- **App** (`internal/app/gateway`) — catalog / provider / router / sink 같은 런타임 조립 책임. `cmd/llmgate` 가 CLI entrypoint 와 signal/shutdown 실행에 집중하도록 부팅 wiring 을 분리한다.
...
| App | gateway | catalog 모델을 provider 로 만들고 router input 과 result sink 를 조립하는 부팅 wiring |
