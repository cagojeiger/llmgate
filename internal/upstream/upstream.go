...
// rawBodyLimit caps the raw-body bytes preserved on *provider.Error.Raw
...
// *provider.Error.Raw (and audit logs), and we don't want them holding
...
// refused, deadline exceeded) into a *provider.Error with the right
// Kind so callers don't have to sniff strings. ProviderName is stamped
// onto the error so audit logs and fallback policy can distinguish
// vendor sources.
func LowLevelError(providerName, message string, cause error) *provider.Error {
...
// *provider.Error with KindBadRequest. raw is trimmed via FirstBytes
// so audit logs stay bounded.
func BadRequest(providerName, message string, cause error, raw []byte) *provider.Error {
	return &provider.Error{
