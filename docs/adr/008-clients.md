# ADR 008: 호출자 등록 — `clients/` sibling, 해시만 yaml, 닫힘 default
...
**호출자는 `clients/` 별도 디렉토리. 키는 sha256 해시만 yaml. 닫힘 default. audit 는 인증 실패도 발행.**

- **`clients/` sibling** — `catalog/` 와 동등한 root. 단일 우산 안에 묶지 않는다. mount 도 따로 (`LLMGATE_CLIENTS=<dir>`). vendor catalog 와 caller roster 가 다른 권한 / mount / 변경 주기로 운영될 여지를 처음부터 열어 둔다.
- **client = yaml 1 개** — `clients/<name>.yaml` 파일 1 개가 호출자 1 개. 필드 minimal:
...
- **닫힘 default** — `LLMGATE_CLIENTS` 미설정 + cwd 에 `./clients` 없음 = 부팅 fail (ADR 002 의 catalog 패턴과 동일). 디렉토리 존재하지만 등록된 호출자 0 개 = 부팅 fail. 의도된 *완전 공개* 는 운영자가 의도적 행동 (예: 단일 `public.yaml` 등록 + 키 공유) 으로 표현한다.
- **audit 는 인증 실패도 발행** — 매치 실패 호출도 record 에 들어간다. `client_name=""`, `error_kind=auth`, `status_code=401`. brute-force / 잘못된 키 사용 시도가 *invisible* 하지 않게. ADR 001 *사실만 발행* 정신과 정확히 짝.
- **audit Record 에 client 필드 추가** — `client_name` (매치된 yaml name, 실패 시 ""), `client_key_id` (매치된 해시의 앞 8자, 회전 추적용; 풀 해시 / raw 키는 절대 로그 안 됨). 두 필드면 *누가 / 어떤 키로* 가 후처리에서 가능.
- **이름 규칙** `^[a-z0-9][a-z0-9_-]{0,63}$` — 모델 id / 별명 name 과 정합. lowercase 정규화, 슬래시 / 콜론 / 공백 / 점 거부 (audit 파싱 / 로그 grep 안전). 길이 64자 상한.
- **이름은 영구 식별자, 재사용 금지** — 폐기된 client name 을 새 호출자에 재배정하지 않는다. 게이트웨이는 검증하지 않는다 (상태 없음 — ADR 001) — 운영자 책임. 후처리 시스템이 client_name 으로 비용 / 호출량 집계하므로 재사용은 *옛 호출 + 새 호출* 을 한 client 로 합쳐 보게 한다 (별명 name 과 같은 자세).
...
- **`catalog/clients/` (catalog 안에 sibling)** — 의미 충돌 (clients 는 catalog 가 아님). mount 한 군데로 묶이면서 권한 분리 awkward. 폐기.
- **raw 키 yaml** — catalog 가 시크릿 저장소가 됨. ADR 002 *yaml = 메타, env = 시크릿* 정신 위반. configmap mount 시 키 노출. 폐기.
- **env per client (`LLMGATE_CLIENT_<name>_KEY`)** — 호출자 수 늘면 env 폭발. *등록* 카테고리에 env 를 쓰는 비대칭. 폐기.
...
- **열림 default (clients 비어있으면 인증 면제)** — 사고 위험. 잠금 default 가 안전. 폐기.
- **client name 자동 ID 보강 (uuid)** — 이름 + 별도 영구 ID 분리. minimal 정신 위반, 별명 name 과 비대칭. V1 미적용.

## 운영 흐름

```
호출자 등록
  → clients/<name>.yaml 작성
  → raw 키 생성 (운영자 도구) → sha256 해시 → key_hashes 에 추가
  → raw 는 호출자에 1 회 전달, 게이트웨이는 raw 안 봄
  → 다음 deploy 시 재시작 (ADR 001 의 "재시작 적용")

키 회전
  → clients/<name>.yaml 의 key_hashes 에 새 해시 추가 → deploy
  → 두 키 동시 유효 구간에 호출자가 새 키로 전환
  → audit 의 client_key_id 로 옛 키 사용 잔량 추적
  → 옛 해시 제거 → deploy → 회전 완료

호출자 폐기
  → clients/<name>.yaml 삭제 → deploy → 그 호출자 모든 키 무효
