...
| 메트릭 | 낮은 cardinality label 정책 + `/metrics` bearer token 옵션 | local 외 환경에서는 token 필수. 네트워크 통제는 운영 증적 필요 |
| result event | `metadata_only` 기본값, `redacted`/`full` 명시 선택, drop/publish failure metric | consumer별 export 승인과 retention/파기 증적 필요 |
...
현재 result event는 원문을 포함할 수 있지만, 기본값은 `metadata_only`다. 원문이 필요한 운영자는 `LLMGATE_LLMRESULT_PAYLOAD_MODE=full`을 명시해야 하고, 부분 구조만 필요한 경우 `redacted`를 쓴다. 이는 stdout 로그보다 훨씬 강한 개인정보/기밀정보 통제 대상이다.
...
- result event drop/publish 실패는 Prometheus metric으로 드러난다.
...
- `internal/platform/config/config.go`: `LLMGATE_LLMRESULT_PAYLOAD_MODE` 기본값은 `metadata_only`.
- `internal/domain/llmresult/sink/async.go`: queue full/closed 시 drop observer를 호출함.
- `internal/platform/telemetry/prometheus`: `llmgate_llmresult_events_dropped_total` metric.
- `internal/platform/nats/llmresult/publisher.go`: publish 실패 시 observer를 호출함.
- `internal/platform/telemetry/prometheus`: `llmgate_llmresult_events_publish_failed_total` metric.
...
`/metrics`는 business auth/middleware 밖에 있지만, 별도 bearer token 옵션이 있다. local compose는 NATS, NATS monitoring, Prometheus, Grafana를 host port로 연다. local-only 의도는 문서에 있지만 프로덕션 운영 증적은 저장소 밖에 남아야 한다.

비어 있는 것:

- local compose의 Grafana anonymous admin은 local-only이나, prod 금지 규칙은 코드/문서에 명확히 고정되어 있지 않다.

근거:

- `internal/platform/http/server/server.go`: `/metrics`가 middleware 밖에 mount되지만 `LLMGATE_METRICS_BEARER_TOKEN`이 있으면 bearer token을 요구함.
- `docs/metrics.md`: 외부 노출 제어는 ServiceMonitor/network policy/ingress 책임.
- `internal/platform/config/config.go`: local 외 환경에서 metrics token 필수, NATS URL은 `tls://` 필수, NATS user/password 필수.
...
- `docs/security/02-operations.md`에서 prod 금지 compose 설정을 명시.

### 3. 감사 로그를 접속기록 기준으로 확장

현재 audit는 “누가, 어떤 모델을, 어떤 결과로 호출했는가”를 잘 남긴다. access log는 `remote_addr`, `user_agent`, `request_id`를 남겨 접속기록 점검의 보조 필드로 쓸 수 있다. 다만 개인정보 안전성 확보조치 기준의 접속기록/점검 관점에서는 운영자가 점검할 보관 주기와 내부 관리계획이 더 필요하다.
...
- `docs/security/02-operations.md`: 최소 보관기간과 점검 책임 기준.
...
| 우선순위 | 항목 | 이유 |
|---|---|---|
| P0 | result event retention | 원문 request/response 경계는 mode/redaction으로 축소됨. drop/publish failure metric도 추가됨. 보관/파기 증적은 남음 |
| P0 | prod 운영 surface 증적 | NATS TLS/auth 및 metrics token은 fail-fast로 보강됨. Grafana/Prometheus 운영 증적은 남음 |
| P1 | audit retention/trusted proxy/점검 주기 | access source fields와 운영 보관 기준은 추가됨. proxy 신뢰 경계와 점검 증적은 남음 |
