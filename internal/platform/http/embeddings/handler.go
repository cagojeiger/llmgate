...
// EmbeddingService is the upstream contract Handler needs.
type EmbeddingService interface {
	Embed(ctx context.Context, req *llmtypes.EmbeddingRequest) (*routing.EmbedResult, error)
}
...
func NewHandler(service EmbeddingService, log *slog.Logger, events telemetry.EventSink, cfg HandlerConfig) *Handler {
	if log == nil {
		log = slog.Default()
	}
	if events == nil {
	...
	// building result events entirely — FromEmbedding deep-clones the full
	// embedding payload, which is real work to construct just to drop.
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var req *llmtypes.EmbeddingRequest
	var resultResp *llmtypes.EmbeddingResponse
	defer func() {
		...
		if h.results != nil {
			if ev, ok := llmresultschema.FromEmbedding(llmresultschema.EmbeddingBuildInput{
				Audit:    rec,
				Call:     call,
				Request:  req,
				Response: resultResp,
	...
	req, requestBytes, err := decodeRequest(w, r, h.maxRequestBytes)
	if err != nil {
		adoptError(rec, err)
	...
	resultResp = h.serveEmbedding(w, r, req, rec, call)
}
