...
| auth middleware | `Authorization: Bearer` 추출 → sha256 → clients Store lookup → ctx 에 ClientInfo 기록. 실패해도 short-circuit 안 함 — handler 가 audit-always emit (ADR 008) |
...
| clients Store | 부팅 시 yaml → sha256 → client 매핑 read-only. 0 개면 부팅 fail (ADR 008) |
| ProbeState | SIGTERM 시 `MarkShuttingDown()` → readiness 만 503. liveness · in-flight 영향 없음 |
| Audit Recorder | 요청당 1 개 fact record. client_name + client_key_id + auth_error 포함 — *누가 / 왜 실패* 의 사실 |
...
clients/                     호출자 등록 (운영자, 코드 0줄)
  <name>.yaml                name + key_hashes (sha256 only)
internal/catalog/            yaml → Catalog 로더
internal/clients/            yaml → Store (sha256 → client lookup)
...
scripts/gen-client.sh        호출자 발급 헬퍼
...
- **clients**: `scripts/gen-client.sh` 가 raw 키 발급 → sha256 만 yaml 에 박음. multi-key 활성 가능 (회전 윈도우). 부재 / 빈 디렉토리 → 부팅 fail (닫힘 default). 결정 근거 ADR 008.
...
| `LLMGATE_CLIENTS` | `./clients` | clients 디렉토리 (부재 → fail) |
...
graph TD
    ...
    Policy --> Dispatcher
    Clients[clients/] --> Store[sha256 → name store]
    ServerCfg --> Server
    Store --> Server
    Dispatcher --> Server
...
순서 (`cmd/llmgate/main.go`): env 로드 → catalog 파싱 → adapter factory → Dispatcher 조립 → clients 파싱 (0 개면 fail) → Audit + Handler + middleware wire → ProbeState + Listen.
...
sequenceDiagram
    ...
    R-->>H: Result
    H-->>A: 200 OK
    H->>Au: Record (client_name + key_id + (auth_error?))
```
...
| 호출자 등록 (해시만) | `clients/` yaml | 외부 갱신 시 재시작 |
...
| 호출자 lookup | clients Store (per-process) | 프로세스 수명 |
