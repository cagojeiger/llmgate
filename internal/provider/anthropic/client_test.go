...
func collectStreamToolCalls(t *testing.T, stream provider.Stream) (map[int]*streamToolCallAcc, string) {
	t.Helper()
...
func TestCompleteStream_ToolUse_Standard(t *testing.T) {
	...
	stream, err := c.CompleteStream(context.Background(), &provider.Request{
		Model:    "claude-x",
		Messages: []provider.Message{{Role: "user", Content: "what time?"}},
	})
	if err != nil {
		t.Fatalf("CompleteStream: %v", err)
	}
...
func TestCompleteStream_ToolUse_MultipleCalls(t *testing.T) {
	...
	stream, err := c.CompleteStream(context.Background(), &provider.Request{
		Model:    "claude-x",
		Messages: []provider.Message{{Role: "user", Content: "do both"}},
	})
	if err != nil {
		t.Fatalf("CompleteStream: %v", err)
	}
