...
- `catalog/models/<id>.yaml` = 모델 한 개의 운영 데이터. 벤더, 프로토콜, endpoint, 인증 env, 벤더별 기본 요청값과 선택적 토큰 단가를 담는다. 정확한 schema는 [data.md](../data.md)가 맡는다.
...
- **카탈로그가 판단하지 않는 것**: alias 이름의 의미, chain 순서의 타당성, 단가의 최신성, context window, 모델 적합도 ([ADR 000](000-identity.md)). 선택적 단가는 upstream 비용이 없을 때 응답을 보완하는 운영 데이터일 뿐 라우팅 정책에는 관여하지 않는다.
...
- 라우팅에 쓰지 않는 일반 모델 메타(context window 등)는 카탈로그 밖이다. 비용 호환을 위한 선택적 단가만 예외로 두며, [ADR 000](000-identity.md)의 작은 실행기 경계를 유지한다.
