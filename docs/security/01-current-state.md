...
`llmgate`는 DB나 앱 소유 영속 저장소를 사용하지 않는다. 직접 저장하는 개인정보 테이블은 없고, 보안 검토의 저장 경계는 stdout을 수집하는 외부 로그 시스템과 선택적으로 켜는 NATS/JetStream result event다.

## Summary

| 영역 | 현재 상태 | 판단 |
|---|---|---|
| 앱 소유 저장소 | DB/Redis/object storage 없음 | 직접 저장보다는 외부 sink 보안 경계가 중요 |
...
| result event | NATS publish 활성화 시 원문 request/response 포함, 앱 레벨 payload 암호화 없음 | 원문 export 승인, NATS 접근통제, retention/파기 증적 필요 |
...
- `llmgate`는 audit/call log를 DB에 저장하지 않고 stdout JSON으로만 내보낸다. 실제 보관은 Loki/ELK 같은 외부 수집기 책임이다.
...
현재 result event는 `LLMGATE_LLMRESULT_NATS_URL`이 설정된 경우에만 외부 NATS/JetStream으로 발행된다. 이 이벤트는 원문 request/response를 포함하고, 현재 앱 레벨 payload 암호화는 하지 않는다. 따라서 이 기능을 켠 환경에서는 stdout 로그보다 훨씬 강한 개인정보/기밀정보 통제 대상이다.

비어 있는 것:

- consumer별 result export opt-in/deny 정책이 없다.
- result event payload 자체를 암호화하는 기능이 없다.
...
- `internal/app/gateway/results.go`: NATS URL이 비어 있으면 `NopSink`를 사용해 result event를 원격 저장하지 않음.
...
- 원문 event가 필요한 환경만 `LLMGATE_LLMRESULT_NATS_URL`을 켜고, NATS TLS/계정/stream 접근통제를 운영 증적으로 남긴다.
...
| 우선순위 | 항목 | 이유 |
|---|---|---|
| P0 | result event retention | 원격 publish 활성화 시 원문 request/response를 durable event로 내보냄. 앱 레벨 payload 암호화는 없으므로 NATS 접근통제와 보관/파기 증적 필요 |
